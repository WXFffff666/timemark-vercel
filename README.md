...
| 内存占用 | ~800MB | **~256MB** |
| 部署复杂度 | 需配置数据库连接 | **开箱即用** |
| 安全加固 | 基础认证 | **MASTER_KEY 强制校验 + JWT + 限流 + XSS 防护** |
| 通知账户 | 明文存储 | **AES 加密存储** |
| 多账户支持 | 单渠道单账户 | **同渠道多账户** |
...
- **安全加固**：MASTER_KEY 启动强制校验，通知渠道凭证 AES 加密存储，JWT 密钥长度强制检查
- **性能优化**：内存占用降低 70%，启动速度提升，适合 J4125 等低功耗 NAS 处理器
- **多账户通知**：同一通知渠道可配置多个账户，灵活分发提醒消息
...
| 🗓️ 精准农历 | 📢 多渠道通知 | 👨‍👩‍👧‍👦 智能关系映射 | 🔒 企业级安全 | 🌍 全球时区 |
|:----------:|:----------:|:---------------:|:----------:|:--------:|
| 闰月自动转换 | 27+ 通知渠道 | 称呼智能适配 | TOTP + JWT 双认证 | NTP 自动同步 |
| 公历/农历双历 | 同渠道多账户 | 家庭关系映射 | MASTER_KEY 加密 | 自定义时区 |
...
TimeMark 支持 27+ 通知渠道，覆盖国内外主流通讯平台。
...
# 生成 MASTER_KEY (64 位十六进制字符串)
openssl rand -hex 32
```

> ⚠️ **MASTER_KEY 为必填项**。未设置 MASTER_KEY 时，应用将拒绝启动并输出致命错误。该密钥用于加密存储通知渠道的敏感凭证（API Key、Token 等），请妥善保管，丢失将导致已加密的凭证无法解密。
...
| 特性 | 说明 |
|------|------|
| **MASTER_KEY 强制校验** | 启动时检查 MASTER_KEY 是否设置，未设置则拒绝启动 |
| **JWT 会话管理** | Access Token (15 分钟) + Refresh Token (7 天)，自动续期 |
| **密钥长度校验** | JWT_SECRET 最小 32 字符，生产环境强制检查 |
| **登录锁定** | 连续 5 次登录失败，自动锁定账户 15 分钟 |
| **API 限流** | 请求频率限制，防止暴力破解和滥用 |
| **XSS 防护** | 输出转义 + 内容过滤，防止跨站脚本攻击 |
| **密码加密** | bcrypt (cost=10) 哈希存储，不可逆 |
| **2FA 双因素** | TOTP 认证，支持 Google/Microsoft Authenticator |
...
| 版本 | 日期 | 内容 |
|:----:|:----:|------|
| **v2.0.0** | 2026-04 | 架构重构：PostgreSQL + Redis → SQLite 单容器；MASTER_KEY 强制校验；通知凭证 AES 加密；多账户通知；触发日志 API |
