...
在 **固定联系人** 页可为每人配置多条邮箱、手机及 IM 账号，每条可填**标签**（如「工作」「妈妈」）。保存后写入 `contact_methods`（JSONB，迁移 v30），并与事件提醒、批量邮件联动。

### 快捷发信流程

```
联系人页 → 邮件图标 →（多邮箱时）勾选收件人 → 编辑主题/正文 → 发送
```

| 场景 | 行为 |
|------|------|
| 仅 1 个邮箱 | 直接进入编辑界面，自动选中该邮箱 |
| 多个邮箱 | 先进入「选择收件邮箱」二级界面，**默认不全选**，需手动勾选 |
| 发送渠道 | 优先使用联系人绑定的邮件类渠道，否则使用第一个可用邮件账号 |

### 安全说明

- 所有 API 经 **HTTPS** 传输；SMTP 外发使用 TLS（587 强制 STARTTLS）。
- 服务端校验 `recipientEmails` **必须属于该联系人**，无法通过 API 向任意第三方邮箱中继。
- 通知渠道 **token/secret** 在列表 API 中已脱敏，编辑时留空表示不修改原密钥。
- 邮件正文**不写入**日志；日志仅记录收件人、主题、状态。
...
除 Resend 外，可在 **通知渠道 → SMTP 邮件** 使用大厂邮箱发信。邮件从你的真实邮箱发出，对 Gmail/QQ 收件箱通常更友好。

### 配置步骤

1. 选择 **邮箱服务商**（自动填充 SMTP 服务器与端口）
2. 填写 **发件人邮箱**（完整地址，兼作登录用户名）
3. 填写 **授权码 / 应用专用密码**（不是网页登录密码）
4. 点击 **测试 SMTP 连接** 确认通过后再保存
5. 在 **批量邮件** 或事件提醒中选择该 SMTP 账号发送

### 各服务商授权码获取

| 服务商 | SMTP | 端口 | 密码说明 |
|--------|------|------|----------|
| QQ 邮箱 | smtp.qq.com | 465 | 邮箱设置 → 账户 → 开启 SMTP → 生成**授权码** |
| 163 邮箱 | smtp.163.com | 465 | 设置 → 开启 SMTP → **客户端授权码** |
| Gmail | smtp.gmail.com | 587 | Google 账号开启两步验证 → **应用专用密码** |
| Outlook | smtp.office365.com | 587 | 登录密码；若开启 MFA 需应用密码 |
| 腾讯企业邮 | smtp.exmail.qq.com | 465 | 完整企业邮箱 + 邮箱密码/客户端密码 |
| 阿里企业邮 | smtp.mxhichina.com | 465 | 完整企业邮箱 + 邮箱密码 |

### Resend 域名与 DMARC

- 发信域验证通过后，发件人应使用 `@email.yourdomain.com`，勿用 `onboarding@resend.dev`
- DMARC 记录可仅写：`v=DMARC1; p=none`（**不必**填写个人邮箱到 `rua=`）
