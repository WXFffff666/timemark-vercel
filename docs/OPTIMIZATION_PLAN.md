...
### 固定联系人与待办
- **多联系方式**：`contact_methods` JSONB（v30）；标签化邮箱/手机/IM
- **快捷发信**：单邮箱直达；多邮箱二级勾选
- **待办完成**：`todo_completions`（v29）；打勾同步服务端
- **完成历史**：过期自动移出当前列表，历史页可查阅；365 天清理

### 日历
- 年 / 月 / 日视图；可点击日期；默认本月事件列表

### 安全
- 联系人发信收件人白名单；通知账号 API 密钥脱敏；HSTS；SMTP requireTLS
