...
## 6. Google 日历 OAuth 同步（C5）

在 **设置 → 集成** 点击「连接 Google 日历」，完成 OAuth 授权后，系统会加密存储 refresh token，并通过 Cron `/api/cron/calendar-sync` 从 **primary** 日历只读导入事件。

| 环境变量 | 说明 |
|----------|------|
| `GOOGLE_OAUTH_CLIENT_ID` | Google Cloud OAuth 客户端 ID |
| `GOOGLE_OAUTH_CLIENT_SECRET` | 客户端密钥 |
| `GOOGLE_OAUTH_REDIRECT_URI` | 可选；默认 `{站点}/api/calendar/google-oauth/callback` |

同步策略与外部 ICS 相同（`add_only` / `replace`），由 `external_calendar_sync_strategy` 控制。
